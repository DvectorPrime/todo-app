...
export default function SubTaskComponent({subTask, todo, todos, setTodos}: SubTaskComponentProps){
    ...
    function updateTodo() : void{
        ...
        const subTaskIndex: number = newTodo.subtasks?.findIndex(subTaski => subTaski.subId == subTaskId) ?? -1

        if (newTodo.subtasks && subTaskIndex !== -1) {
    ...
    return(
        <>
            <div></div>
            <label htmlFor={subTask.subId} className="flex align-center gap-4 cursor-pointer">
